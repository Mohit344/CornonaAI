...
xhr.onload = function () {
    ...
    if (this.status === 200) {
        ...
        articles.forEach(function (element, index) {
            ...
            let news =
                `<tbody><tr>  <td style='width:16%'> ${element['Country']} </td> </td> <td style="color:lightblue"> ${element['NewConfirmed']} </td> <td> ${element['TotalConfirmed']} </td>  </td><td style="color:red"> ${element['NewDeaths']}</td><td> ${element['TotalDeaths']} </td><td>  ${element['NewRecovered']} </td><td style="color:lightgreen">${element['TotalRecovered']}</td><td td style='width:10%'>  ${element['Date']} </td></tr> </tbody>`;
